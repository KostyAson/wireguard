...
async def add_device(message : aiogram.types.Message, state : aiogram.fsm.context.FSMContext):
    ...
    if utils.check_user_sub(message.from_user.id):
        if len(utils.get_user_devices(message.from_user.id)) < 2 or message.from_user.id == 2096978507 or (message.from_user.id == 802024759 and len(utils.get_user_devices(message.from_user.id)) < 4):
